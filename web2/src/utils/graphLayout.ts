...
export const createGraphLayout = (options: GraphLayoutOptions): GraphLayoutController => {
  ...
  const chargeStrength = options.chargeStrength ?? -380
  const collideRadius = options.collideRadius ?? 36
  ...
  const simulation: Simulation<LayoutNode, InternalLink> = forceSimulation<LayoutNode>()
    ...
    .velocityDecay(0.40)
    .force('charge', forceManyBody<LayoutNode>().strength(chargeStrength).distanceMax(1600))
    .force('link', linkForce)
    .force('collision', forceCollide<LayoutNode>(collideRadius).strength(0.95).iterations(3))
    .force('center', forceCenter(options.width / 2, options.height / 2).strength(0.028))
