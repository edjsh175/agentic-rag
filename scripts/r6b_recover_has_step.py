"""R6b: salvage safe Procedure/Step (conf>=0.80) then recover has_step edges."""
...
SOURCE_BATCHES = [
    "7c848f88-a994-4162-8dc9-87af94df3214",  # R6
    "a6309a3d-6bab-484c-b919-74d121dc399a",  # R5
]
# R6 rejected most Steps at 0.80; keep leaf threshold slightly looser for recovery only.
ENTITY_MIN_CONF = 0.80
REL_MIN_CONF = 0.80
PREFER = {"Procedure": 0, "Step": 1}
...
def collect(db: RelationalDB) -> tuple[list[dict], list[dict], dict]:
    ...
    for batch_id in SOURCE_BATCHES:
        batch = db.get_extraction_batch(batch_id) or {}
        for c in db.list_extraction_candidates(batch_id, "rejected"):
            ...
            if not is_safe_review_candidate(c, batch=batch):
                skip["entity_unsafe_or_diagnostic"] += 1
                continue
            ...
            entity_items.append(
                {
                    ...
                    "evidence_text": c.get("evidence_text") or p.get("evidence_text") or "",
                    "source_batch": batch_id,
                    "source_candidate_id": c["id"],
                }
            )

    # prefer Procedure if same name appears as both (shouldn't after seen_entity)
    sim = dict(name_type)
    for item in entity_items:
        p = item["payload"]
        sim[normalize_entity_name(p.get("name") or "")] = p.get("entity_type")
    ...
    Path("data/llm_backfill_r6b_has_step_recoverable.json").write_text(
        json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    print(json.dumps({k: summary[k] for k in ("entity_count", "rel_count", "entity_by_type", "skip")}, ensure_ascii=False, indent=2))
...
def main() -> None:
    ...
    if not entity_items and not rel_items:
        print("nothing to recover")
        return
    if not rel_items:
        print("WARNING: salvaging entities but no has_step unlocked yet; still applying entities")

    db_path = Path(resolve_db_path())
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    backup = Path("data/backups") / f"rag_relational_pre_r6b_has_step_{ts}.db"
    backup.parent.mkdir(parents=True, exist_ok=True)
    shutil.copy2(db_path, backup)
    print("backup", backup)

    bid = db.create_extraction_batch(
        "llm_has_step_recovery",
        {
            "note": "R6b salvage Procedure/Step>=0.80 then has_step",
    ...
    db.review_extraction_candidates(
        bid, approve_ids, "approved", "r6b recover has_step chain"
    )
    ...
    Path("data/llm_backfill_r6b_has_step_apply_summary.json").write_text(
