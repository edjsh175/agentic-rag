...
> 基线：2026-07-20；**进度复核：2026-07-28**。本说明以 `rag_knowledge/`、`web/src/`、`config.ini` / `config-prod.ini`、`run.py` 和 `rag_knowledge/api/routes.py` 为准。
>
> 图中的“可选”表示代码已经接入、但当前配置关闭；“外部”表示由本项目调用或代理而非本项目持久化。
> **Graph Retrieval**：本地 `config.ini` 可开全图（含融合上限）；**生产模板 `config-prod.ini` 默认关**。Reranker 生产 CPU 默认关。
...
flowchart TD
    ...
    K --> FUSE
    FUSE --> STRUCT["结构化加权\n表格、表头、章节命中（启用）"]
    STRUCT --> GRAPH{"Graph Retrieval\n本地可开 / 生产模板关"}
    GRAPH -->|"是"| GR["实体链接、图侧 Chunk、融合上限\n（图块≤1 + 保护文本 top1）"]
    GRAPH -->|"否"| RERANK
    GR --> RERANK["BGE / Cross-Encoder Reranker\n（本地可开 / 生产 CPU 默认关）"]
...
| 能力 | 本地 config.ini / 生产模板 | 架构含义 |
|---|---|---|
| 主检索 | hybrid + rrf | 同时使用 Chroma 和 BM25，再做 RRF 融合 |
| Reranker | 本地可开 / 生产默认关 | 粗召回后 Cross-Encoder 精排 |
| Query Planner | 开启 | 流程、排错、比较等问题可扩大召回与邻居范围 |
| 结构化检索 | 开启 | 表格、表头和章节命中加权 |
| Graph Retrieval | 本地开 / 生产关 | 图侧 Chunk 融合 + 融合上限；不替代 Chunk 事实来源 |
| 图谱 LLM 抽取 | CLI 按需 | 建设写路径；禁止无差别盲抽「其他」 |
