...
- **关联第 3 轮**：[`../待执行-第3轮-安全全量重建/执行PRD.md`](../待执行-第3轮-安全全量重建/执行PRD.md) / [第3轮执行验收记录.md](../待执行-第3轮-安全全量重建/第3轮执行验收记录.md)
- **关联问答主线**：[`../进行中-主干锚定检索与关系可答/执行PRD.md`](../进行中-主干锚定检索与关系可答/执行PRD.md) / [`../已完成-锚点约束Chunk过滤与干扰对照实验/执行PRD.md`](../已完成-锚点约束Chunk过滤与干扰对照实验/执行PRD.md)
...
**不会**把关系 `evidence_text` 当可引用事实写入回答契约。第 4 轮 GraphRAG 仍延后，**不挡**本 PRD。
...
5. 【延后】第 4 轮 GraphRAG A/B；全库 graph_retrieval.enabled=true
```
