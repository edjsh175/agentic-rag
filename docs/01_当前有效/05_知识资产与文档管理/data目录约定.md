...
- 导航以 [知识图谱PRD剩余轮次总览](../03_知识图谱与GraphRAG/2026-07-13-知识图谱PRD剩余轮次总览.md) 与 [第4轮阶段总结](../../04_已完成归档/03_知识图谱与GraphRAG/已完成-第4轮-GraphRAG实效验收/2026-07-27-阶段总结.md) 为准。
- **图谱语义抽取（2026-08-11）**：验收与日常抽图主路径仅 `RAG_CONFIG=config-local.ini`（Ollama + `[graph_extraction.llm] num_ctx/num_predict`）；`config-mix.ini` 外部抽取仅遗留摸底，不得结项。详见 [示例学习与模型本地化 PRD](../../04_已完成归档/03_知识图谱与GraphRAG/已完成-抽取路径本地化-示例学习/2026-08-11-图谱语义抽取示例学习与模型本地化PRD.md)。
