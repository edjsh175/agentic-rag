# 切块基石治理 — Round 0B 并行准备包

在 Round 0B 收尾期间并行完成的 **评测准备** 与 **0C/0E/留痕设计 + 离线 Spike**。

> **编号权威**：以母 PRD **v1.2.2**（`../2026-07-14-RAG文档解析与切块基石治理PRD.md`）§7 / §13 / §14 为准。  
...
| `lineage_manifest_contract.md` | **Round 0C** 留痕 | 契约冻结 + Spike ✅；生产 run 目录待补 |
...
| `multi_chunk_qa_gold_v2` + `fr10_baseline_*` | **Round 0F** 评测基线 | ✅（冻结旧库基线） |
| 受控重建 | **Round 0G** | **部分完成**（人工豁免上线；见 GOV-0G-20260716-001） |

## 硬边界（历史记录；0C/0G 已执行）

并行准备阶段曾约定：不写正式库、不改 Loader 主路径。上述边界在 **2026-07-16 0G 重建与豁免切换后已解除**；当前 live 状态以 [GOV-0G-20260716-001](../../已完成-第0C轮-切块合并与隔离验证/GOV-0G-20260716-001-门禁未过人工豁免上线.md) 为准。
...
留痕运行产物目录：历史 spike 在 `data/archive/chunk_audit/_spike_*`；新跑默认仍可写 `data/chunk_audit/`（gitignore，含原文），验收后迁入 archive。
...
| 本目录设计与基线 | 并行包已交付 |
| Round 0C | 生产接线 + 隔离验收 ✅ |
| Round 0G | 已重建；**人工豁免上线**；门禁账见 [GOV-0G-20260716-001](../../已完成-第0C轮-切块合并与隔离验证/GOV-0G-20260716-001-门禁未过人工豁免上线.md) |
| 下一步 | 黄金 Section 锚点迁移 → 0F → 图谱小批 |
