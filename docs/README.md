...
3. **反问指称唯一性与二次开发直达**（Phase 0 已落地，FR-7 未做）：点名合法锚零反问，废除「种子 ≥ 2 ⇒ 反问」。[08-14 PRD](3_待办清单/知识图谱语义抽取/待执行-反问指称唯一性与二次开发直达/2026-08-14-反问指称唯一性与二次开发直达整改PRD.md)。
...
未完成 PRD 集中在 [`3_待办清单/待执行/`](3_待办清单/待执行/)；切块 0D/0E、图谱 08-14 路径段、08-14 反问指称 FR-7（Phase 0 已落地）仍在各自专题的 `待执行-*` 目录。
