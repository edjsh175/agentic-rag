...
3. **路径段类型独占与功能区短名展示**（待实施）：[08-14 PRD](3_待办清单/知识图谱语义抽取/待执行-路径段类型独占与功能区短名展示/2026-08-14-路径段类型独占与功能区短名展示PRD.md)。
4. **四层架构叶子挂载**：功能区候选+人工确认、`mount_status`；禁止以 `endswith("表")` 作挂载主路径。[PRD](3_待办清单/待执行/2026-07-31-知识图谱四层架构与领域语义模型升级PRD.md)（双图切换等已部分落地）。
5. **legacy Profile 自动瘦身**。
6. **切块 0D / 0E**；全量 `--mode qa` 生成式批测（FR-10 是检索路径冻结，**08-07 10 题模型选型评测不能替代**）。

### P2（交付与运维）

7. 部署风险、Excel 复杂 fixture、前端布局剩余项（Dagre 已接，未用 G6）等见 [待办清单](3_待办清单/待办清单.md)。

未完成 PRD 集中在 [`3_待办清单/待执行/`](3_待办清单/待执行/)；切块 0D/0E、图谱 08-14 仍在各自专题的 `待执行-*` 目录。
