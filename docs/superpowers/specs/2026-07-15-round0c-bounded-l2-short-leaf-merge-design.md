...
2026-07-15 按本设计完成实现与三手册隔离复测，报告位于 `docs/3_待办清单/chunk基石治理/chunk-foundation-round0c-l2-isolation/`。
...
随后补齐生产链路中的超长文本切分、过滤后邻接重算，以及同 Section 表格相邻说明的显式 `table_context` 标记。按 PRD §8.2 的普通文本口径复测结果位于 `docs/3_待办清单/chunk基石治理/chunk-foundation-round0c-final-isolation/`：
