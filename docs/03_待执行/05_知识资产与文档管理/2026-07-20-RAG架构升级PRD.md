...
> **进度复核（2026-08-20）**：**待执行 / 战略架构规划**。本 PRD 仍未按本文整体落地；当前未发现 `DocumentTreeService`、`document_versions`、`IngestionJob`、`index_snapshots` 等核心主数据体系。现有 `kb_path` / file index / Chroma / SQLite 运行链路不能视为本文目标架构已实现。现行进度见 [待办清单.md](../../01_当前有效/00_项目总览与架构/待办清单.md)。
