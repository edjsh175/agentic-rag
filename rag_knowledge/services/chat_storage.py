...
class ChatStorage:
    ...
    def _get_lock(self, fingerprint: str) -> threading.Lock:
    ...
    def _load_raw(self, fingerprint: str) -> dict:
        """加载原始 JSON 并自动执行平滑迁移（调用方应在锁保护下执行）"""
    ...
    def _save_raw(self, fingerprint: str, data: dict) -> None:
        """保存数据到 JSON 文件（采用临时文件 + 原子替换，调用方需在锁保护下执行）"""
        fp = self._path(fingerprint)
        data["updated_at"] = datetime.now().isoformat(timespec="seconds")
        content = json.dumps(data, ensure_ascii=False, indent=2)
        tmp_file = fp.with_name(f"{fp.stem}_{uuid.uuid4().hex[:8]}.tmp")
        try:
    ...
    def list_sessions(self, fingerprint: str) -> dict:
        ...
        with self._get_lock(fingerprint):
    ...
    def save_session(
        ...
        """保存指定会话的消息与标题。若会话不存在则返回 None（严格更新语义）"""
        with self._get_lock(fingerprint):
            raw = self._load_raw(fingerprint)
            now_str = datetime.now().isoformat(timespec="seconds")
            target = None
            ...
            if target is None:
                logger.warning("保存会话失败，会话不存在: %s / %s", fingerprint, session_id)
                return None
    ...
    def save(self, fingerprint: str, messages: list) -> dict:
        ...
        with self._get_lock(fingerprint):
            ...
            if not active_id or not any(s.get("id") == active_id for s in raw.get("sessions", [])):
                ...
                raw.setdefault("sessions", []).insert(0, new_sess)
                raw["active_session_id"] = active_id
                self._save_raw(fingerprint, raw)
                return new_sess
            else:
                target = next(s for s in raw["sessions"] if s.get("id") == active_id)
                target["messages"] = messages
